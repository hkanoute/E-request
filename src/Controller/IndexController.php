...
class IndexController extends AbstractController
{
    ...
    public function photo(SuppT200Repository $repository, Request $request, EntityManagerInterface $em): Response
    {


        $form = $this->createForm(Image360Type::class, null, []);

        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {

            $artnr = $form->get('reference')->getData();

            $data = $repository->getImagesByArtnr($artnr);

            return $this->render('index/photo.html.twig', [
                'form' => $form->createView(),
                'data' => $data,
            ]);
        }
